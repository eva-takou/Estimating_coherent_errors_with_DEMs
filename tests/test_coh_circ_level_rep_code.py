#Test the coherent circuit-level noise model for the repetition code memory. 
#We estimate hyperedges and use the expressions to redefine the error rates.
...
def estimate_coh_DEM(detection_events,d,rds,include_higher_order):
...
def verify_coh_model(ITERS,include_higher_order):
    ...
    ds = [3,5,7,9]
    # thetas = np.linspace(0.01*np.pi,0.1*np.pi,5)
    thetas = np.array([0.043*np.pi, 0.047*np.pi, 0.05*np.pi, 0.052*np.pi, 0.054*np.pi  ]) 
    # thetas  = np.array([ 0.03*np.pi, 0.04*np.pi, 0.05*np.pi, 0.06*np.pi ]) 
    ps     = [100*np.sin(th)**2 for th in thetas]
    ...
    for d in ds:
        ...
        for theta in thetas:
            ...
            DEM = estimate_coh_DEM(detection_events,d,rds,include_higher_order)
...
# verify_coh_model(ITERS=10**6,include_higher_order=include_higher_order)
...
def _run_single_point(d, theta, ITERS, include_higher_order,
                      p, q_readout, Reset_ancilla):
...
def verify_coh_model(ITERS, include_higher_order, n_jobs=-1):
...
include_higher_order = True
verify_coh_model(ITERS=3*10**5,include_higher_order=include_higher_order)
