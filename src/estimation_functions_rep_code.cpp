...
Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic> cast_as_Eigen_batch(const std::vector<std::vector<uint8_t>>& batch) {
    ...
    Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic> flattened_batch(nsims, total_len);
    for (int i = 0; i < nsims; ++i) {
        for (int j = 0; j < total_len; ++j) {
            flattened_batch(i, j) = static_cast<bool>(batch[i][j]);
        }
    }
...
std::vector<Real> calculate_vi_mean(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, int nsims, int n_stabs, int rds){
    ...
    for (int rd=0; rd<rds; ++rd){
        ...
        for (int anc=0; anc<n_stabs; ++anc){
            ...
            auto batch_indx = flattened_batch.col(indx);
...
inline Real get_prob(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, int rd1, int rd2, int anc1, int anc2, int nsims, int n_stabs){
    ...
    auto joint_counts =  flattened_batch.col(indx1) && flattened_batch.col(indx2);
...
//I'm not entirely sure about this function.
...
inline Real get_four_point_prob(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, int nsims, int n_stabs, int rds,
    ...
    auto col1 = flattened_batch.col(indx1);
    auto col2 = flattened_batch.col(indx2);
    auto col3 = flattened_batch.col(indx3);    
    auto col4 = flattened_batch.col(indx4);    
...
inline Real get_three_point_prob(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, int nsims, int n_stabs, int rds,
...
//These are all 4-point probs for repetition code, where we have 2 consecutive dets in same rd, and 2 consecutive in the next rd
ProbDict get_all_four_point_probs(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, 
                                  int nsims, int n_stabs, int rds){

    
    ProbDict four_point_probs; //(total_bits)
    ...
    for (int rd=0; rd<rds-1; rd++){
        ...
        for (int anc=0; anc<n_stabs-1; anc++){
            ...
            Real pijkl = get_four_point_prob(flattened_batch, vi_mean, nsims, n_stabs, rds,
...
ProbDict get_all_three_point_probs(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, int nsims, int n_stabs, int rds,
    ...
    for (int rd=0; rd<rds-1; ++rd){
        ...
        for (int anc=0; anc<n_stabs-1; ++anc){
            ...
            Real pijk=get_three_point_prob(flattened_batch, vi_mean,  nsims,  n_stabs,  rds,
...
std::vector<Real> estimate_time_edges(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, 
    ...
    for (int rd1=0; rd1<rds-1; ++rd1){
        ...
        for (int anc1=0; anc1<n_stabs; ++anc1){
            ...
            Real p = get_prob(flattened_batch, vi_mean,  rd1,  rd2,  anc1,  anc2,  nsims, n_stabs);   
            ...
            // std::vector<uint64_t> mask2 = make_mask(lower_order_key, total_bits);
            ...
            //If it is last rd and also the first or last ancilla do not modify
            // if (rd2==rds-1 && (anc1==0 || anc1==n_stabs-1)){
            //     p_time.push_back(p);
            //     continue;
            //     int indx1 = anc1 + n_stabs * rd1;
            //     int indx2 = anc2 + n_stabs * rd2;
            //     std::vector<int> lower_order_key = {indx1,indx2};
            //     p = remove_4_point_from_first_or_last_time_edges(lower_order_key, p, three_point_probs);
                
            // }

            // //If it is 1st round then remove only 4-point?
            // if (rd1==0){
            //     int indx1 = anc1 + n_stabs * rd1;
            //     int indx2 = anc2 + n_stabs * rd2;
            //     std::vector<int> lower_order_key = {indx1,indx2};
            //     std::cout << "previous value:" << p << "\n";
            //     p = remove_4_point_from_first_or_last_time_edges(lower_order_key, p, four_point_probs);
            //     // p = remove_4_point_from_first_or_last_time_edges(lower_order_key, p, three_point_probs);
            //     p_time.push_back(p);
            ...
            //     std::cout << "Time value prior to redefining: " << p << "\n";
            //     int indx1 = anc1 + n_stabs * rd1;
            //     int indx2 = anc2 + n_stabs * rd2;
            //     std::vector<int> lower_order_key = {indx1,indx2};
            ...
            //     // std::cout << "Time value after subtracting 4-pnt: " << p << "\n";
            //     p = redefine_lowest_order_prob(lower_order_key, p, three_point_probs);
            //     std::cout << "Time value after subtracting 3-pnt: " << p << "\n";
            //     p_time.push_back(p);
            // }
...
std::vector<Real> estimate_space_edges(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, 
                                       int nsims, int n_stabs, int rds,
                                       const ProbDict& four_point_probs, const ProbDict& three_point_probs){

    std::vector<Real> p_space;
    p_space.reserve(rds * (n_stabs-1));
    std::vector<int> lower_order_key(2);
    // int total_bits = n_stabs*rds;
...
std::vector<Real> estimate_diag_edges(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, 
                                     int nsims, int n_stabs, int rds, const ProbDict& four_point_probs, const ProbDict& three_point_probs){
    ...
    //Let's check the other diagonal as well:
...
std::vector<Real> estimate_bd_edges(const Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic>& flattened_batch, const std::vector<Real>& vi_mean, 
                                    int nsims, int n_stabs, int rds, const ProbDict& four_point_probs, const ProbDict& three_point_probs){
    ...
    for (int rd1=0; rd1<rds; ++rd1){
        ...
        //Nearest SPACE edge
        Real p1 = get_prob(flattened_batch,  vi_mean,  rd1,  rd1, anc1, anc1+1,nsims, n_stabs);  //rd1,rd2,anc1,anc2
        ...
        if (rd1+1<rds){
            ...
            Real p2 = get_prob(flattened_batch,  vi_mean,  rd1,  rd1+1, anc1, anc1, nsims,n_stabs);  //rd1,rd2,anc1,anc2
        ...
        if (rd1-1>=0){
            ...
            Real p3 = get_prob(flattened_batch,  vi_mean,  rd1,  rd1-1, anc1, anc1, nsims,n_stabs); //rd1,rd2,anc1,anc2
        ...
        if (rd1-1>=0){
            ...
            Real p4 = get_prob(flattened_batch,  vi_mean,  rd1,  rd1-1, anc1, anc1+1,nsims,n_stabs);
    ...
    for (int rd1=0; rd1<rds; ++rd1){
        ...
        if (rd1+1<rds){
            ...
            Real p4 = get_prob(flattened_batch,  vi_mean,  rd1,  rd1+1, anc1, anc1-1,nsims,n_stabs); //rd1,rd2,anc1,anc2
...
//TODO: Put an option to exclude/include higher order events --- because otherwise we have to recompile each time.
//TODO: Or make a new function (i.e., one that redefines, one that doesn't redefine.)
//TODO: Put an option to print or not the values.
std::tuple<std::vector<Real>,std::vector<Real>,std::vector<Real>> estimate_edges_rep_code(const std::vector<std::vector<uint8_t>>& batch, int d, int n_stabs, int rds){
    //rds are the effective rds (including or not the stabilizer reconstruction which is controlled externally.)

    int nsims                                                          = batch.size();
    Eigen::Array<bool, Eigen::Dynamic, Eigen::Dynamic> flattened_batch = cast_as_Eigen_batch(batch);

    std::vector<Real>  vi_mean = calculate_vi_mean(flattened_batch, nsims, n_stabs, rds);
    ProbDict four_point_probs  = get_all_four_point_probs(flattened_batch, vi_mean,  nsims,  n_stabs,  rds);
    ProbDict three_point_probs = get_all_three_point_probs(flattened_batch, vi_mean,  nsims,  n_stabs,  rds, four_point_probs);
    ...
    std::vector<Real>  p_space = estimate_space_edges(flattened_batch, vi_mean, nsims, n_stabs, rds, four_point_probs,three_point_probs);
    std::vector<Real>  p_time  = estimate_time_edges(flattened_batch, vi_mean, nsims, n_stabs, rds, four_point_probs, three_point_probs);
    std::vector<Real>  p_diag  = estimate_diag_edges(flattened_batch, vi_mean, nsims, n_stabs, rds,four_point_probs,three_point_probs);
    std::vector<Real>  p_bd    = estimate_bd_edges(flattened_batch, vi_mean, nsims, n_stabs, rds,four_point_probs,three_point_probs);
